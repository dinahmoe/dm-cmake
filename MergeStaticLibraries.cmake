...
# this is executed just if we are using Make.
  ...
  # First get the file names of the libraries to be merged	
    foreach(lib ${libs})
      ...
      if(NOT libtype STREQUAL "STATIC_LIBRARY")
        message(FATAL_ERROR "Merge_static_libs can only process static libraries (${lib} is not)")
      endif()
      if(multiconfig)
        ...
        list(APPEND libfilesNames "${lib}")
      endif(multiconfig)
    endforeach()
    message(STATUS "will be merging ${libfilesNames}")
  ...
  # Now the easy part for MSVC and for MAC
    ...
    elseif(APPLE)
      ...
      message(STATUS "Using general unix mechanism")
    ...
    #---------------------------------					
      ...
      add_custom_command(TARGET ${outlib} POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E echo "Running: ${CMAKE_AR} rP ${outfile} @${objlistfilerpath}"
        COMMAND ${CMAKE_AR} rP "${outfile}" @"${objlistfilerpath}"
        WORKING_DIRECTORY ${objdir})		
    endforeach()
    add_custom_command(TARGET ${outlib} POST_BUILD
    ...
    SET(outlib_variable ${outlib})
    STRING(REGEX REPLACE "-" "_" outlib_variable "${outlib_variable}")
    file(WRITE ${dummyfile}.base "const char* ${outlib_variable}_sublibs=\"${libs}\";")
    add_custom_command( 
      OUTPUT  ${dummyfile}
      COMMAND ${CMAKE_COMMAND}  -E copy ${dummyfile}.base ${dummyfile}
      DEPENDS ${libs} ${extrafiles})
  ELSE(CMAKE_GENERATOR MATCHES "^Unix Makefiles")
    ADD_DEPENDENCIES(${outlib} ${libs})
